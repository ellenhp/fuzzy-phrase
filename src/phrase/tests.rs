...
fn contains_query() {
    ...
    assert_eq!(
        &phrase_set.get_by_id(Output::new(2)).unwrap(),
        &[561_528u32, 1u32, 61_528_u32]
    );
...
fn contains_prefix_range() {
    let mut build = PhraseSetBuilder::memory();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[2u8, 1u8, 0u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[2u8, 3u8, 2u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[2u8, 3u8, 4u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[2u8, 5u8, 6u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[4u8, 1u8, 1u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[4u8, 3u8, 3u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[4u8, 5u8, 5u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[6u8, 3u8, 4u8])])
        .unwrap();
    build
        .insert(&[1u32, 61_528_u32, three_byte_decode(&[6u8, 3u8, 7u8])])
        .unwrap();
    build
...
fn get_full(phrase: &str) -> Vec<QueryWord> {
    phrase
        .split(' ')
        .map(|w| QueryWord::new_full(WORDS[w], 0))
        .collect::<Vec<_>>()
}

fn get_prefix(phrase: &str) -> Vec<QueryWord> {
    let words: Vec<&str> = phrase.split(' ').collect();
    let mut out = words[..(words.len() - 1)]
        .iter()
        .map(|w| QueryWord::new_full(*WORDS.get(w).unwrap(), 0))
        .collect::<Vec<QueryWord>>();
    let last = &words[words.len() - 1];
    let prefix_match = WORDS
        .iter()
        .filter(|(k, _v)| k.starts_with(last))
        .collect::<Vec<_>>();
    out.push(QueryWord::new_prefix((
        *prefix_match[0].1,
        *prefix_match.last().unwrap().1,
    )));
    out
}
...
fn get_expected_range(encoded_phrase: &[QueryWord], exact_match: bool) -> Option<(u64, u64)> {
    let matches = |id_phrase: &Vec<u32>| {
        if (exact_match && id_phrase.len() != encoded_phrase.len())
            ...
            for (i, qw) in encoded_phrase.iter().enumerate() {
                let matches = match qw {
                    ...
                    QueryWord::Prefix {
                        id_range: (start, end),
                        ..
                    } => *start <= id_phrase[i] && *end >= id_phrase[i],
                };
                if !matches {
                    return false;
                }
    ...
    match seek {
        Some(start) => {
            let len = ID_PHRASES[start..]
                .iter()
                .take_while(|m| matches(*m))
                .count();
...
fn sample_contains() {
    ...
    for phrase in PHRASES.iter() {
        assert!(SET.lookup(&get_full(phrase)).found_final());
    }
}
...
fn sample_doesnt_contain() {
    ...
    for phrase in PHRASES.iter() {
        ...
        inverse.reverse();
        assert!(!SET.lookup(&inverse).found_final());
    }

    // a couple manual ones
    let contains = |phrase| SET.lookup(&get_full(phrase)).found_final();
...
fn sample_contains_prefix() {
    ...
    let contains_prefix = |phrase| SET.lookup(&get_prefix(phrase)).found();
...
fn sample_check_prefix_ranges() {
    ...
    assert_eq!(
        get_range("84"),
        get_expected_range(&get_prefix("84"), false)
    );
    assert_eq!(
        get_range("84#"),
        get_expected_range(&get_prefix("84#"), false)
    ...
    assert_eq!(
        get_range("84# G"),
        get_expected_range(&get_prefix("84# G"), false)
    );
    assert_eq!(
        get_range("84# Suchava Dr"),
        get_expected_range(&get_prefix("84# Suchava Dr"), false)
    );
}
...
fn sample_check_continuations() {
    ...
    assert_eq!(
        true,
        SET.lookup(&get_prefix("84# Gleason Hollow"))
            .has_continuations()
    );
    assert_eq!(
        false,
        SET.lookup(&get_prefix("84# Suchava Dr"))
            .has_continuations()
    );
    assert_eq!(
        false,
        SET.lookup(&get_prefix("84# Gleason Suchava"))
            .has_continuations()
    );
}
...
fn get_full_variants(phrase: &str) -> Vec<Vec<QueryWord>> {
    phrase
        ...
        .map(|w| {
            DISTANCES[&WORDS[w]]
                .iter()
                .map(|(id, distance)| QueryWord::new_full(*id, *distance))
                .collect::<Vec<_>>()
        })
        .collect::<Vec<_>>()
}
...
fn get_prefix_variants(phrase: &str) -> Vec<Vec<QueryWord>> {
    ...
    let prefix_match = WORDS
        ...
        .filter(|(k, _v)| k.starts_with(last))
        .collect::<Vec<_>>();
    let mut last_group = vec![QueryWord::new_prefix((
        *prefix_match[0].1,
        *prefix_match.last().unwrap().1,
    ))];
...
fn sample_match_combinations() {
    ...
    let no_typo = SET
        .match_combinations(&get_full_variants("53# Country View Dr"), 1)
        .unwrap();
    assert!(no_typo == vec![correct.clone()]);

    let typo = SET
        .match_combinations(&get_full_variants("53# County View Dr"), 1)
        .unwrap();
...
fn sample_match_combinations_as_prefixes() {
    ...
    let no_typo1 = SET
        .match_combinations_as_prefixes(&get_prefix_variants("53# Country"), 1)
        .unwrap();
    assert!(no_typo1 == vec![correct1.clone()]);

    let typo1 = SET
        .match_combinations_as_prefixes(&get_prefix_variants("53# County"), 1)
        .unwrap();
    assert!(typo1 != vec![correct1.clone()]);

    let correct2 = get_prefix_combination("53# Country V");
    let no_typo2 = SET
        .match_combinations_as_prefixes(&get_prefix_variants("53# Country V"), 1)
        .unwrap();
    assert!(no_typo2 == vec![correct2.clone()]);

    let typo2 = SET
        .match_combinations_as_prefixes(&get_prefix_variants("53# County V"), 1)
        .unwrap();
...
fn sample_contains_windows_simple() {
    ...
    for phrase in PHRASES.iter() {
        ...
        let results = SET
            .match_combinations_as_windows(&word_possibilities, max_phrase_dist, ends_in_prefix)
            .unwrap();
...
fn sample_match_combinations_as_windows_all_full() {
    ...
    for phrase in PHRASES.iter() {
        ...
        let results = SET
            .match_combinations_as_windows(&word_possibilities, max_phrase_dist, true)
            .unwrap();
...
fn sample_prefix_contains_windows_overlap() {
    ...
    let results = SET
        .match_combinations_as_windows(&word_possibilities, 1, true)
        .unwrap();
...
fn sample_prefix_contains_windows_substring() {
    ...
    let results = SET.match_combinations_as_windows(&word_possibilities, 1, false);
    ...
    let results = SET
        .match_combinations_as_windows(&word_possibilities, 1, false)
        .unwrap();
    ...
    assert_eq!(
        results,
        vec![get_full_window("59 Old New Milford Rd", false)]
    );
